...
export const Route = createFileRoute("/construo")({
  head: () =>
    seo({
      title: "O que eu construo — sistemas web, APIs, automação e IA · Henrique Kenzo",
      description:
        "Quatro frentes de desenvolvimento de software: backend e APIs (Node.js, Python), front-end (React, Next.js), bancos de dados e automação com integrações e IA.",
      path: "/construo",
    }),
...
const cats = [
  ...
  },
  {
    ...
    items: [
      "Customizações de Sistemas via JS",
                ...
                <Reveal key={c.title} delay={i * 0.06}>
                  <article className="group relative h-full overflow-hidden border border-border bg-white/[0.02] p-8 transition-colors hover:border-primary/40">
                    <div className="flex items-start justify-between mb-6">
                      <div
                        className={`size-10 ${c.color === "primary" ? "bg-primary/15 text-primary" : "bg-accent/15 text-accent"} grid place-items-center`}
                      >
                    ...
                    <ul className="space-y-2 mb-8">
                      {c.items.map((it) => (
                        <li key={it} className="flex items-center gap-3 text-sm">
                          <span
                            className={`size-1.5 ${c.color === "primary" ? "bg-primary" : "bg-accent"}`}
                          />
                    ...
                    <div className="flex flex-wrap gap-1.5 border-t border-border pt-5">
                      {c.tech.map((t) => (
                        <span
                          key={t}
                          className="font-mono text-[10px] uppercase tracking-widest text-foreground/70 bg-white/5 border border-border px-2 py-1"
                        >
                          {t}
                        </span>
                      ))}
                    ...
                    <div
                      className={`absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent ${c.color === "primary" ? "via-primary" : "via-accent"} to-transparent opacity-0 group-hover:opacity-60 transition-opacity`}
                    />
                  </article>
                </Reveal>
              ))}
