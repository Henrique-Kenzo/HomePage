...
export const Route = createFileRoute("/projetos/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: loaderData?.project
      ? [
          { title: `${loaderData.project.name} · Estudo de caso` },
          { name: "description", content: loaderData.project.tagline },
          { property: "og:title", content: `${loaderData.project.name} — ${loaderData.project.category}` },
          { property: "og:description", content: loaderData.project.tagline },
          { property: "og:image", content: dashImg },
        ]
      : [],
  }),
  notFoundComponent: () => (
    <div className="min-h-screen grid place-items-center bg-background text-foreground">
      <div className="text-center">
        <p className="font-mono text-xs uppercase tracking-widest text-muted mb-4">404 // PROJECT NOT FOUND</p>
  ...
  errorComponent: () => (
    <div className="min-h-screen grid place-items-center bg-background text-foreground">
      <div className="text-center">
        ...
        <Link to="/projetos" className="text-primary underline">Voltar</Link>
      </div>
    </div>
  ),
      ...
      <div className="relative z-10">
        ...
        <section className="px-6 pt-20 pb-12">
          <div className="mx-auto max-w-7xl">
            <Reveal>
              <Link to="/projetos" className="font-mono text-[10px] uppercase tracking-[0.25em] text-muted-foreground hover:text-primary">← Projetos</Link>
              ...
              <Reveal>
                <div className="relative group">
                  ...
                  <div className="relative overflow-hidden rounded-2xl border border-white/10 shadow-[0_30px_100px_-20px_rgba(0,0,0,0.9)]">
                    ...
                    <img src={dashImg} alt={`Dashboard do ${project.name}`} width={1536} height={1024} loading="eager" className="block h-auto w-full" />
                  </div>
                </div>
              </Reveal>
            </div>
          ...
          <div className="mx-auto max-w-6xl grid gap-12 md:grid-cols-2">
            <Reveal>
              <div className="border-l border-primary pl-6">
                <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-primary mb-3">01 // Problema</div>
                <p className="text-foreground/90 text-lg leading-relaxed">{project.problem}</p>
              </div>
            </Reveal>
            <Reveal delay={0.05}>
              <div className="border-l border-primary pl-6">
                <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-primary mb-3">02 // Solução</div>
                <p className="text-foreground/90 text-lg leading-relaxed">{project.solution}</p>
              </div>
            </Reveal>
            <Reveal delay={0.1}>
              <div className="border-l border-accent pl-6">
                <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-accent mb-3">03 // Arquitetura</div>
            ...
            <Reveal delay={0.15}>
              <div className="border-l border-accent pl-6">
                <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-accent mb-3">04 // Resultado</div>
          ...
          <div className="mx-auto max-w-6xl">
            <Reveal>
              ...
              <div className="flex flex-wrap gap-2">
                {project.stack.map((t) => (
                  <span key={t} className="font-mono text-xs uppercase tracking-widest text-foreground/80 bg-white/5 border border-border px-3 py-1.5">{t}</span>
                ))}
              </div>
            </Reveal>
          </div>
